...
var eventoFormCheck = () => {
    let titreSel:any = $("input[name='titrevento']");
    ...
    let descSel:any = $("textarea[name='summaryevento']");
...
function traiteReponseAjax(response:any , dataPost:any) {
...
function domMiseajourEventoHTML(urlEvento:any) {
    let titre:any = $("input[name='titrevento']").val();
    let desc:any = $("textarea[name='summaryevento']").val();
...
function eventoDatasRequest(args:any) {

    let idxChecked = $('#modalEventoCreneaux li > input:checked');

    let questions:Array<any> = [];

    idxChecked.each(function(this: any) {
    ...
    let jsonData:any = Object.assign({}, eventoDraftBase);
    let propositionBase:any = Object.assign({}, jsonData.questions[0].propositions[0]);
    ...
    if (jsonData.notify_new_guests == true) {
        ...
        if (agendasDistants.length > 0) {
          agendasDistants.forEach((data) => {
...
function eventoAjaxSurvey(datas:any, type:any, url:any, traiteReponseAjax:any) {
    let settings = {
        async: true,
        type: type,
